...
def show_results(counts, reset_callback, unsafe_password=None):
    ...
    if show:
        st.header("Traffic")
        st.write(f"<span style='color:red'>since {counts['start_time'][0:35]} To Today</span>", unsafe_allow_html=True)
        ...
        # Ensure that the lengths of the lists in per_day are equal
        max_length = max(len(counts["per_day"]["days"]), len(counts["per_day"]["pageviews"]), len(counts["per_day"]["script_runs"]))
        counts["per_day"]["days"] = counts["per_day"]["days"] + [""] * (max_length - len(counts["per_day"]["days"]))
        counts["per_day"]["pageviews"] = counts["per_day"]["pageviews"] + [0] * (max_length - len(counts["per_day"]["pageviews"]))
        counts["per_day"]["script_runs"] = counts["per_day"]["script_runs"] + [0] * (max_length - len(counts["per_day"]["script_runs"]))
        ...
        with st.expander("Advanced Options"):
            ...
            if reset_prompt == "I'm absolutely sure that I want to reset the results":
                ...
                password=st.text_input("Please enter the password to confirm", type="password")
                if password == "Muneesh K":
                    reset_callback()
